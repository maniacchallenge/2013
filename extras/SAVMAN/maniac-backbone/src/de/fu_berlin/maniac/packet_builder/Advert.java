...
public class Advert extends Packet {
	...
	private void parseFromByteArray(byte[] payload) {
		...
		for (int i = 0; i < 4; i++){
			address[i] = payload[5 + i];
			       //System.err.println(" ==> parseFromBteArray" + address[i]);
		}
		try {
			this.finalDestination = (Inet4Address) InetAddress
					.getByAddress(address);
			// System.err.println(" ==> finalDestination: " + this.finalDestination);
		} catch (UnknownHostException e) {
			       System.err.println(" ==>  Unknown Host Exception in parseFromByteArray");
	...
	private byte[] parseToByteArray() {
		...
		//System.err.println(" ==> " + destinationIP);
		...
		if (finalDestination == null) {
			...
			byte[] address = finalDestination.getAddress();
			 //System.err.println("parsetoByteArray ==> finalDestination: " + this.finalDestination);
			for (int i = 0; i < 4; i++){
