...
public final class Auction implements Runnable {
	...
	public void run() {
		...
		System.err.println("[" + new Date() + "]" +"[" + this.packetbuilder.getDeviceIP() +  "] Auction Manager: ending the auction at: " + df.format(System.currentTimeMillis()));
		System.out.println("[" + new Date() + "]" +"[" + this.packetbuilder.getDeviceIP() +  "] Auction Manager: trying to select winner for:" + pack.getTransactionID());
		...
		if(!strat.dropPacketAfter(pack)){
			...
			System.out.println("[" + new Date() + "]" +"[" + this.packetbuilder.getDeviceIP() +  "] Auction Manager: Received Bids: " + bids.size());
			...
			for(int i = 0; i < bids.size(); i++){
				...
				bidId = i; 
                                                           ...
                                                           System.out.println("[" + new Date() + "]" +"[" + this.packetbuilder.getDeviceIP() +  "] Auction Manager: Winner is Bid: " + bidId + " with Value: " + lowest);
                                                              
                                                           Bid winner = bids.get(bidId);
			
			if (winner != null) {
				System.out.println("[" + new Date() + "]" +"[" + this.packetbuilder.getDeviceIP() +  "] Auction Manager: Sending BidWins to all");
				sender.send(packetbuilder.buildBidWin(winner, pack.getFine()));
				packetbuilder.updateData((Data) pack, winner.getSourceIP());
				System.out.println("[" + new Date() + "]" +"[" + this.packetbuilder.getDeviceIP() +  "] Auction Manager: Sending Data to: " + winner.getSourceIP());   
				sender.send(pack);
			} else {
				System.err.println("[" + new Date() + "]" +"[" + this.packetbuilder.getDeviceIP() +  "] No Winner => Backbone: ");
				/*
