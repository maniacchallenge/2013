...
/**
 * Main control-class doing packet processing and calling the user strategy. All incoming packets go through this class.
 * Depending on the type, Mothership will do whatever is necessary, i. e. calls your onReceiveAdvert() if an
 * Advert-Packet for you comes in.
 *
...
public class Mothership extends Thread {
          ...
          /**
           * Amount of time the program waits until it chooses a winner for the Auction, in ms
           */
          ...
          Sender sender;
          /**
           * Packet queue where the Receiver puts the packets and Mothership polls it to get them
           */
          LinkedBlockingQueue<Packet> packetqueue;
          /**
           * This Hashtable keeps track of all the transactionIDs of Data packets that this node has won (so the node
           * can not bid for Data it has already won previously
           */
          ...
          BidDelayer delayer;
          /**
           * All received packets are put in this ArrayList so the UI (Test app) can display incoming traffic. For test
           * purposes, not important for functionality
           */
          ...
          public Mothership() throws SocketException {
	         receiver = new Receiver();
	         receiver.start();
	         sender = new Sender();
	         packetqueue = receiver.getPacketQueue();
	         won_transactionIds = new Hashtable<Integer, Boolean>(50);
          ...
          public void run() {
	         ...
	         while (true) {
		        ...
		        if (buffer_packet == null) {
			       ...
			       /**
			        * Packet handling occurs here, not all packets are passed to the user strategy
			        */
			       switch (buffer_packet.getType()) {
				      ...
				      case 'A':
					     try {
						    ...
						    System.out.println("[" + new Date() + "]" + "[" + TopologyInfo.getOwnIP() + "] Received an Advert");
						    System.out.println(buffer_adv.toString());

						    /**
						     * Add the adverts AuctionParameters to the list
						     * of received adverts
						     */
						    ...
						    if (isForUser(buffer_adv)) {
							   System.out.println("[" + new Date() + "]" +"[" + TopologyInfo.getOwnIP() + "] Advert was for me");
							   /**
							    * If the advert is for the user, call
							    * onRcvAdvert() and delay the bid sent
							    * out by the return value.
							    */
				      ...
				      case 'B':
					     ...
					     System.out.println("[" + new Date() + "]" + "[" + TopologyInfo.getOwnIP() + "] Received a Bid: ");
					     System.out.println(buffer_bid.toString());
					     /**
				      ...
				      case 'W':
					     ...
					     System.out.println("[" + new Date() + "]" +  " [" + TopologyInfo.getOwnIP() + "] Received a BidWin");
					     System.out.println(buffer_bidwin.toString());
					     /**
				      ...
				      case 'D':
					     ...
					     System.out.println("[" + new Date() + "]" + "[" + TopologyInfo.getOwnIP() + "] Received Data");
					     ...
					     if (!dropPacket) {
						    ...
						    /**
						     * If the data packet is destined for the backbone
						     * associated with this node, send it there.
						     */
						    if (buffer_data.getFinalDestination().equals(myOwnBackbone)) {
							   packetbuilder.updateData(buffer_data,
								(Inet4Address) myOwnBackbone);
							   sender.send(buffer_data);
						    } /**
						     * If the data packet hasnt been won before AND
						     * the user does not want to drop it, start an
						     * auction:
						     */
							 ...
							 .getTransactionID())) {
							   ...
							   /**
							    * Get the AuctionParameters with which
							    * the data packet should be auctioned
							    * from the user
							    */
							   parameters = strategy.onRcvData(buffer_data);

							   /**
							    * If user returns null then take the
							    * winning bid for this data as maxBid
							    * and the fine from the last auction
							    * (included in data)
							    */
							   if (parameters == null) {
								  ...
								  /**
								   * If the user return was not
								   * null, update the fine in
								   * the data packet
								   */
							   ...
							   /**
							    * If fine is bigger than the fine from
							    * last auction, set it to fine from
							    * last auction
							    */
							   ...
							   System.err.println("[" + new Date() + "][ " + TopologyInfo.getOwnIP() + "] Registering and starting auction at: " + new Date()  );
					     ...
					     /**
					      * Remember that this data has been processed so it cannot
					      * be bid on in the future!
					      */
          ...
          /**
           * A advert is for the user if the data packet being advertised has not already been won by this node at some
           * other auction and also if the advert is either from this nodes associated backbone OR any other node. (So
           * that excludes advert from other backbones)
           *
           * @param dg_pck
           * @return
          ...
          public boolean isForUser(Packet dg_pck) throws UnknownHostException {
	         ...
	         if (!won_transactionIds.containsKey(dg_pck.getTransactionID())) {

		        return true;
	         }
	         System.out.println("[" + new Date() + "] Packet has already been won by this node at an auction. No further action");
	         return false;
          }

          /**
           * Use this function to give Mothership your strategy. DO THIS BEFORE SENDING OR RECEIVING OR ANYTHING ELSE!
