...
public class PacketBuilder implements PacketBuilderInterface {
	...
	private PacketBuilder(Inet4Address IP,TopologyInfo tp) {
		this.tp = tp;
		// SIM: We know the Ip better ;D
		this.deviceIP = IP;	
		//deviceIP = (Inet4Address) tp.getInterfaceIpv4("wlan0");
		broadcastAddr = (Inet4Address) tp.getBroadCastAddress(deviceIP);
	}
	
	public static PacketBuilder getInstance(Inet4Address IP, TopologyInfo tp) {
		/* Break Singleton for Simulator (First call from Mothership break only */
		System.err.println("NEW PACKETBUILDER => " + IP);
		instance = new PacketBuilder(IP,tp);
		return instance;
	}       
