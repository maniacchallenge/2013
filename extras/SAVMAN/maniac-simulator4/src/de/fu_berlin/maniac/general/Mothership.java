...
import de.tuhh.maniac.simulator.Backbone;
...
public class Mothership extends Thread {
	...
	public Mothership(Inet4Address IP) throws SocketException {
                       /*                             
	...
	*/
		this.IP = IP;
		System.out.println("["+ IP + "] Mothership lift off");
                                            // SIM: The order is crucial!
                                            TopologyInfo tp = new TopologyInfo(IP);
                                             packetbuilder = PacketBuilder.getInstance(IP,tp);
                                             netman = NetworkManager.getInstance(IP,tp,packetbuilder);       
		...
		recv_adverts = new Hashtable<Integer, AuctionParameters>(50);
		backbones = netman.getBackbones();
		/* Removed parameter new ManiacLoger(con) */
					...
					if(!dropPacket){
						...
						/** If the data packet hasnt been won before AND the user does not want to drop it, start an auction:*/
							...
							auctionThreads.schedule(new Auction(buffer_data,
									...
									this.myOwnBackbone, this.packetbuilder), AUCTION_TIMEOUT,
									TimeUnit.MILLISECONDS);
